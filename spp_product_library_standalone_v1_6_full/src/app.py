...
class ProductLibraryApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SPP Product Library - Standalone v1.9.4 Clean Layout")
    ...
    def build_ui(self):
        ...
        self.tree = ttk.Treeview(center, columns=cols, show="tree headings", height=18, style="Product.Treeview")
        ...
        for c, title, w in [
            ...
            ("size", "Size", 95),
            ("price", "Price", 75),
            ("orientation", "Orientation", 80),
            ("printer", "Printer", 110),
            ("press_temp", "Press Temp", 80),
            ("press_time", "Press Time", 80),
        ]:
            self.tree.heading(c, text=title)
            self.tree.column(c, width=w, minwidth=60)

        # Keep important text visible in smaller windows.
        self.tree.column("name", stretch=True)
        self.tree.column("category", stretch=True)
        for _fixed_col in ("size", "price", "orientation", "printer", "press_temp", "press_time"):
            self.tree.column(_fixed_col, stretch=False)
