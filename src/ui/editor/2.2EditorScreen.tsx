...
function ImageStudio({
  ...
  async function applyParamsNow(params: EngineParams): Promise<void> {
    if (!asset) return;

    const requestVersion = ++applyVersionRef.current;

    // Persist params only when committing the control, not on every slider tick.
    onPatch({
      metadata: {
        ...layer.metadata,
        imageEditParams: params as unknown as import("@/types/primitives").JsonValue
      }
    });
    ...
    try {
      ...
      // Prevent older/slower Python results from overwriting newer slider changes.
      if (updated && requestVersion === applyVersionRef.current) {
        onUpdateAsset(updated);
      }
    } finally {
      if (requestVersion === applyVersionRef.current) {
        setApplying(false);
      }
    }
  }

  function updateLocalParam(key: string, value: number | boolean): EngineParams {
    const nextParams: EngineParams = {
      ...localParams,
      [key]: value
    };
  ...
  function commitParams(params: EngineParams): void {
    void applyParamsNow(params);
  }

  function updateAndCommitParam(key: string, value: number | boolean): void {
    const nextParams = updateLocalParam(key, value);
  ...
  return (
    <>
      ...
      <div className="studio-tab-bar">
        <button
          className={`studio-tab${studioTab === "adjustments" ? " active" : ""}`}
          type="button"
          onClick={() => setStudioTab("adjustments")}
        >
      ...
      {studioTab === "adjustments" && (
        <>
          ...
          {!canApply && (
            <div style={{ padding: "4px 10px", fontSize: 11, color: "#f59e0b", opacity: 0.95 }}>
              מנוע עריכת התמונה לא זמין כרגע. הערכים נשמרים, אבל לא מוחלים על התמונה עד שחיבור Python/Electron יעבוד.
            </div>
          )}
                ...
                <div key={p.key} style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      value={val}
                      onChange={(e) => updateLocalParam(p.key, +e.target.value)}
                      onMouseUp={(e) => commitParams({ ...localParams, [p.key]: Number(e.currentTarget.value) })}
                      onPointerUp={(e) => commitParams({ ...localParams, [p.key]: Number(e.currentTarget.value) })}
                      onTouchEnd={(e) => commitParams({ ...localParams, [p.key]: Number(e.currentTarget.value) })}
                      onBlur={(e) => commitParams({ ...localParams, [p.key]: Number(e.currentTarget.value) })}
                      onKeyUp={(e) => commitParams({ ...localParams, [p.key]: Number(e.currentTarget.value) })}
              ...
              <div key={cb.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
                  <input
                    ...
                    onChange={(e) => updateAndCommitParam(cb.key, e.target.checked)}
                  />
                  {cb.label}
                </label>
              </div>
