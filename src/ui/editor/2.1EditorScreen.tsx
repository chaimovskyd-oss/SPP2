...
function ImageStudio({
  ...
  async function applyParamsNow(params: EngineParams): Promise<void> {
    ...
    // Persist params immediately, even if Python is unavailable.
    onPatch({
      metadata: {
        ...layer.metadata,
  ...
  function scheduleApply(params: EngineParams): void {
    if (debounceTimerRef.current !== null) {
      window.clearTimeout(debounceTimerRef.current);
    }
  ...
  function updateParam(key: string, value: number | boolean): void {
    ...
    onPatch({
      metadata: {
        ...layer.metadata,
        imageEditParams: nextParams as unknown as import("@/types/primitives").JsonValue
      }
    });

    // Apply automatically after the user stops moving/changing the control.
    scheduleApply(nextParams);
  }
