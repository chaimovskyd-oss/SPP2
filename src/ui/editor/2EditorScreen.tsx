...
import { isImageEditorAvailable, openImageEditorForAsset, applyImageParams, isHeadlessProcessingAvailable } from "@/services/imageEditorService";
...
// ─── Image Studio — Full Python Engine Integration ────────────────────────────

// Category/param definitions matching adjustment_pipeline.py + i18n.py Hebrew labels
const ENGINE_CATEGORIES = [
  {
    key: "quick", icon: "🪄", label: "מהיר",
    params: [
      { key: "exposure", label: "חשיפה ×100", min: -200, max: 200, step: 1, default: 0 },
      { key: "brightness", label: "בהירות", min: -100, max: 100, default: 0 },
      { key: "contrast", label: "קונטרסט", min: -100, max: 100, default: 0 },
    ],
  },
  {
    key: "light", icon: "💡", label: "אור",
    params: [
      { key: "highlights", label: "אזורים בהירים", min: -100, max: 100, default: 0 },
      { key: "shadows",    label: "צללים",          min: -100, max: 100, default: 0 },
      { key: "whites",     label: "לבנים",           min: -100, max: 100, default: 0 },
      { key: "blacks",     label: "שחורים",          min: -100, max: 100, default: 0 },
      { key: "gamma",      label: "גאמה",            min: -100, max: 100, default: 0 },
    ],
  },
  {
    key: "color", icon: "🎨", label: "צבע",
    params: [
      { key: "temperature", label: "טמפרטורה",     min: -100, max: 100, default: 0 },
      { key: "tint",        label: "גוון",           min: -50,  max: 50,  default: 0 },
      { key: "vibrance",    label: "חיות צבע",      min: -100, max: 100, default: 0 },
      { key: "saturation",  label: "רוויה",          min: -100, max: 100, default: 0 },
    ],
    checkboxes: [{ key: "black_white", label: "שחור לבן" }],
  },
  {
    key: "portrait", icon: "👤", label: "פורטרט",
    params: [
      { key: "ai_face_brighten",       label: "הבהרת פנים",         min: 0, max: 100, default: 0 },
      { key: "ai_face_restore",        label: "שחזור פנים",          min: 0, max: 100, default: 0 },
      { key: "ai_skin_tone_protection",label: "הגנת גוון עור",       min: 0, max: 100, default: 0 },
      { key: "print_reduce_red_skin",  label: "הפחתת אדמומיות עור",  min: 0, max: 100, default: 0 },
      { key: "ai_subject_enhance",     label: "שיפור נושא",          min: 0, max: 100, default: 0 },
    ],
  },
  {
    key: "effects", icon: "✨", label: "אפקטים",
    params: [
      { key: "gaussian_blur",       label: "טשטוש גאוסי",          min: 0,    max: 50,  default: 0 },
      { key: "motion_blur",         label: "טשטוש תנועה",           min: 0,    max: 100, default: 0 },
      { key: "motion_angle",        label: "זווית תנועה",            min: -180, max: 180, default: 0 },
      { key: "radial_blur",         label: "טשטוש רדיאלי",          min: 0,    max: 100, default: 0 },
      { key: "ai_background_blur",  label: "טשטוש רקע",             min: 0,    max: 100, default: 0 },
      { key: "ai_background_darkening", label: "הכהיית רקע",         min: 0,    max: 100, default: 0 },
      { key: "vignette_amount",     label: "וינייט",                 min: -100, max: 100, default: 0 },
      { key: "vignette_feather",    label: "רכות וינייט",            min: 0,    max: 100, default: 65 },
      { key: "vignette_midpoint",   label: "נקודת אמצע וינייט",     min: 0,    max: 100, default: 50 },
      { key: "grain_amount",        label: "גרעיניות",               min: 0,    max: 100, default: 0 },
      { key: "grain_size",          label: "גודל גרעיניות",          min: 1,    max: 60,  default: 18 },
    ],
  },
  {
    key: "advanced", icon: "⚙️", label: "מתקדם",
    params: [
      { key: "sharpness",           label: "חידוד",          min: 0,    max: 100, default: 0 },
      { key: "noise_reduction",     label: "הפחתת רעש",      min: 0,    max: 100, default: 0 },
      { key: "color_noise_reduction",label: "רעש צבע",       min: 0,    max: 100, default: 0 },
      { key: "texture",             label: "טקסטורה",         min: -100, max: 100, default: 0 },
      { key: "clarity",             label: "צלילות",          min: -100, max: 100, default: 0 },
    ],
  },
] as const;

type EngineParams = Record<string, number | boolean>;
...
function ImageStudio({
  ...
  const [studioTab, setStudioTab] = useState<"adjustments" | "tips">("adjustments");
  const [applying, setApplying] = useState(false);

  // All engine params stored per-layer in metadata
  const savedParams = useMemo(
    () => (layer.metadata["imageEditParams"] ?? {}) as EngineParams,
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [layer.metadata["imageEditParams"]]
  );
  const [localParams, setLocalParams] = useState<EngineParams>(() => ({ ...savedParams }));
  ...
  // Get the relevant asset
  const assetId = layer.type === "image" ? layer.assetId : (layer.type === "frame" ? layer.imageAssetId : undefined);
  const asset = assets.find((a) => a.id === assetId);

  const canApply = isHeadlessProcessingAvailable() && asset !== undefined;

  function updateParam(key: string, value: number | boolean): void {
    setLocalParams((prev) => ({ ...prev, [key]: value }));
  }

  async function handleApply(triggerKey?: string): Promise<void> {
    if (!asset) return;
    setApplying(true);
    const params = triggerKey ? { ...localParams, [triggerKey]: localParams[triggerKey] ?? 0 } : localParams;
    // Persist params to metadata immediately
    onPatch({ metadata: { ...layer.metadata, imageEditParams: params as unknown as import("@/types/primitives").JsonValue } });
    if (canApply) {
      const updated = await applyImageParams(asset, params as Record<string, number | boolean | string>);
      if (updated) onUpdateAsset(updated);
    }
    setApplying(false);
  }

  function handleResetAll(): void {
    const empty: EngineParams = {};
    setLocalParams(empty);
    onPatch({ metadata: { ...layer.metadata, imageEditParams: {} } });
  }

  const hasAny = Object.values(localParams).some((v) => v !== 0 && v !== false);
  const cat = ENGINE_CATEGORIES.find((c) => c.key === activeCategory) ?? ENGINE_CATEGORIES[0];

  return (
    <>
      {/* Studio tab switcher */}
      <div className="studio-tab-bar">
        <button className={`studio-tab${studioTab === "adjustments" ? " active" : ""}`} type="button" onClick={() => setStudioTab("adjustments")}>
          <SlidersHorizontal size={12} /> כוונונים
        </button>
        <button className={`studio-tab${studioTab === "tips" ? " active" : ""}`} type="button" onClick={() => setStudioTab("tips")}>
      ...
      {studioTab === "adjustments" && (
        <>
          ...
          <AccordionSection title="שכבה" defaultOpen={true}>
            ...
            <div className="quick-controls">
              <button className={layer.visible ? "toggle on" : "toggle"} onClick={() => onPatch({ visible: !layer.visible })} type="button">
                {layer.visible ? <Eye size={14} /> : <EyeOff size={14} />} תצוגה
              </button>
              <button className={layer.locked ? "toggle on" : "toggle"} onClick={() => onPatch({ locked: !layer.locked })} type="button">
                {layer.locked ? <Lock size={14} /> : <Unlock size={14} />} נעילה
              </button>
            </div>
            <SliderField decimals={2} label="שקיפות" max={1} min={0} step={0.01}
              value={layer.opacity} onChange={(v) => onPatch({ opacity: v } as Partial<VisualLayer>)} />
          </AccordionSection>

          {/* Category nav */}
          <div className="engine-cat-nav" style={{ display: "flex", gap: 3, flexWrap: "wrap", padding: "6px 8px", borderBottom: "1px solid var(--color-border,#2a2a3e)" }}>
            {ENGINE_CATEGORIES.map((c) => (
              <button
                ...
                style={{
                  padding: "4px 7px", borderRadius: 8, fontSize: 15, cursor: "pointer", border: "none",
                  background: activeCategory === c.key ? "var(--color-accent,#7C6FE0)" : "transparent",
                  color: activeCategory === c.key ? "#fff" : "var(--color-text-secondary,#aaa)",
                  lineHeight: 1,
                }}
              >
                {c.icon}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 10px 2px" }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {hasAny && (
                <button className="btn btn-ghost btn-xs" type="button" onClick={handleResetAll} title="איפוס הכל">
                  ↺ איפוס
                </button>
              )}
              {canApply && (
                <button
                  ...
                  style={{ background: "#22c55e", color: "#fff", borderRadius: 6, padding: "2px 8px", fontSize: 12 }}
                  title="החל כל הכוונונים"
                >
                  {applying ? "…" : "✓ החל הכל"}
                </button>
              )}
          ...
          {/* Param sliders */}
          <div className="engine-params-list" style={{ padding: "4px 8px" }}>
            {"params" in cat && cat.params.map((p) => {
              const val = (localParams[p.key] as number | undefined) ?? p.default;
              const isDirty = val !== p.default;
              return (
                <div key={p.key} style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 1 }}>
                      <span style={{ color: isDirty ? "var(--color-accent,#7C6FE0)" : "inherit" }}>{p.label}</span>
                  ...
                  {canApply && (
                    <button
                      type="button"
                      disabled={applying}
                      onClick={() => void handleApply(p.key)}
                      title={`החל ${p.label}`}
                      style={{
                        width: 22, height: 22, borderRadius: 6, border: "none", cursor: applying ? "wait" : "pointer",
                        background: isDirty ? "#22c55e" : "var(--color-surface2,#2a2a3e)",
            ...
            {"checkboxes" in cat && cat.checkboxes?.map((cb) => (
              <div key={cb.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
                  <input
                    type="checkbox"
                    checked={(localParams[cb.key] as boolean | undefined) ?? false}
                    onChange={(e) => updateParam(cb.key, e.target.checked)}
                  />
                  {cb.label}
                </label>
                {canApply && (
                  <button
                    type="button"
                    disabled={applying}
                    onClick={() => void handleApply(cb.key)}
                    style={{ width: 22, height: 22, borderRadius: 6, border: "none", cursor: "pointer", background: "#22c55e", color: "#fff", fontSize: 13 }}
                  >
                    ✓
                  </button>
                )}
