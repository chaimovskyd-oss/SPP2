...
export interface HarmonizeOptions {
  ...
  /** Safe v4: sample only near the object instead of whole canvas */
  localSampleMargin?: number;
  /** Hard safety caps. Prevents black/crushed AI-like results. */
...
export async function runHarmonizePreview(
  ...
  const pythonOptions = {
    // Safe Harmonize v4 defaults. These are intentionally conservative.
    ...
    ...options,
    // Do not allow the UI slider to become destructive. Python caps internally too.
